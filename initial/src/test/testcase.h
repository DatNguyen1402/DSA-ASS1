...
REGISTER_TESTCASE(tc_inventory1005) {
    ...
    List1D<string> queryResult1 = inventory.query("weight", 10, 20, 30, true);
    os << "\nQuery result (weight between 10 and 20, quantity >= 30):" << endl;
    os << queryResult1 << endl;
    List1D<string> queryResult2 = inventory.query("weight", 10, 20, 30, false);
    os << "\nQuery results (weight between 10 and 20, quantity >= 30, descending):" << endl;
    os << queryResult2 << endl;
}
...
REGISTER_TESTCASE(tc_inventory1016) {
    InventoryAttribute attrA1("weight", 15);
    List1D<InventoryAttribute> listAttrA(&attrA1, 1);
    InventoryAttribute attrB1("weight", 15);
    List1D<InventoryAttribute> listAttrB(&attrB1, 1);
    InventoryAttribute attrC1("weight", 15);
    List1D<InventoryAttribute> listAttrC(&attrC1, 1);
    List1D<InventoryAttribute> attributesArray[3] = { listAttrA, listAttrB, listAttrC };
    List2D<InventoryAttribute> attributesMatrix(attributesArray, 3);
    ...
    string namesArray[] = { "Product A", "Product B", "Product C" };
    List1D<string> productNames(namesArray, 3);
    int quantitiesArray[] = { 50, 40, 40 };
    List1D<int> quantities(quantitiesArray, 3);

    InventoryManager inventory(attributesMatrix, productNames, quantities);

    os << "\nQuery (ascending = true):" << endl;
    os << inventory.query("weight", 10, 20, 30, true) << endl;
    os << "Query (ascending = false):" << endl;
    os << inventory.query("weight", 10, 20, 30, false) << endl;
}
